            		...
            		<div class="col-md-12">
		        	  	<div class="card card-preview">
	                        <div class="card-inner">
	                            <table class="table nk-tb-list nk-tb-ulist" data-auto-responsive="false" id="arr_feat_table">
	                                <thead>
...
function edit_action(this_el, item_id){
	...
	var row = TableArrFeat.row(tr_el);
...
function initDataTable() {
    TableArrFeat = $('#arr_feat_table').DataTable({
         processing: true,
         serverSide: true,
         language: { 
         	search: '', 
...
$(document).ready(function(){
	var DataTableArrFeat = initDataTable();
	$('#delete_action').on('click', function (e) {
        e.preventDefault();
        let id = $('#id').val();
        $.ajax({
            url: "{{ url('inavcec/arrfeat/') }}/"+id,
            data: {
                '_token': "{{ csrf_token() }}"
            },
            type: "DELETE",
            success: function (data) {
                ...
                TableArrFeat.ajax.reload(null, false); 
    ...
    $('#edit_action').on('click', function (e) {
        ...
        let id 		= $('#id').val(); 
        let method 	= 'POST';
        let url  	= "{{ url('inavcec/arrfeat/create') }}/";
        if(id.length > 0){
        	method 	= 'PATCH';
        	url 	=  "{{ url('inavcec/arrfeat/') }}/"+id;
        }

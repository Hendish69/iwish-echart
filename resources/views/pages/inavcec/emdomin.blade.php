                                ...
                                <div class="row">
                                	<div class="col-md-6">
                                		<table class="table w-100" id="tb_pie" data-auto-responsive="true" >
										  <thead>
										    <tr>
										      <th scope="col" class="text-center">Type of Flight</th>
            		...
            		<div class="col-md-12">
		        	  	<div class="card card-preview">
	                        <div class="card-inner" style="width:100%">
	                        	<div class="card-title-group mb-3">
                                    <div class="card-title">
                                        <h6 class="title" id="ttl_em_table">Domestic and International Emissions </h6>
                                    </div>
	                            ...
	                            <table class="table nk-tb-list nk-tb-ulist " data-auto-responsive="true" id="last_data_table" style="width:100%">
	                                <thead>
	                                    <tr class="nk-tb-item nk-tb-head">  
                                            <th class="nk-tb-col tb-col-md"><span class="sub-text">Type of Flights</span></th>  
...
$('#btn_show').click(function(){
	...
	if(date_req.length > 0){
		...
		function tbFunctionPie() { 
			$('#ttl_total_em').text('Total Domestic and International Emissions in ' + ttl_em_card);
			initTablePie(request_data+date_req);
		} 

		async function funcDrawPie() {  
			const response = await fetch("{{ route('reportdomin.domPieChart') }}"+"/"+request_data+date_req);
		...
		async function funcDrawBar() {  
			const response = await fetch("{{ route('reportdomin.domgetBarChart') }}"+"/"+request_data+date_req);
		    const data = await response.json(); 
 			sbarchart.update({
			    series: [{
			        name: 'Type fof Flight',
			        colorByPoint: true,
			        data: data.datasBar
			    }]
		    });
		...
		function func3() { 
			$('#ttl_em_table').text('Domestic and International Emission Data in ' + ttl_em_card);
			initDataTable(request_data+date_req);
		} 
		// if (!is_range){
			$('#hourly_tab').removeClass('d-none');
            $('#ttl_hourly').text('Domestic and International Emissions Chart in ' + ttl_em_card);
...
function initDataTable(reqs) {
	...
	if ( $.fn.dataTable.isDataTable( '#last_data_table' ) ) {
		...
		table.ajax.url( "{{ route('reportdomin.domgetData') }}"+'/'+optional ).load();
		table.ajax.reload();
	}else{
         ...
         ajax: "{{ route('reportdomin.domgetData') }}"+'/'+optional,
         columns: [
			{ data: 'TypeOFlight' },// 0 
			{ data: 'Start' },		// 1
			{ data: 'TaxiOut' },	// 2
...
function initTablePie(reqs) {
	...
	if ( $.fn.dataTable.isDataTable( '#tb_pie' ) ) {
		...
		table.ajax.url( "{{ route('reportdomin.domTabelPie') }}"+'/'+optional ).load();
		table.ajax.reload();
	}else{
		 ...
		 lengthMenu: [ 10, 25, 50, 75, 100 ],
         ajax: "{{ route('reportdomin.domTabelPie') }}"+'/'+optional,
         columns: [
			{ data: 'typeofflight' }, 				// 0
			{ data: 'amount' },   

                ...
                <form action="api/airport/chart/save" method="post"  enctype="multipart/form-data" id="chartform">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-2">
                            <strong>Nr</strong>
                            <br>
                            <input type="text" style="text-transform:uppercase" onfocusout="getsourcenr()" class="form-control" id="nr_yr" name="nr_yr">
                        </div>
                        <div class="col-md-3">
...
function chartname(){
        ...
        if (Chartcd == ""){
            ...
            $("#chart_name").val(arp.icao + ' ' + Chartcd.toUpperCase() + this.pgc + ', ' + this.codec +  this.navc + this.rwyt + dteff)
                ...
                '<div class="dropdown">'+
                    ...
                    '<div class="dropdown-menu dropdown-menu-left dropdown-menu-md">'+
                        '<ul class="link-list-plain">'+
                            '<li><a class="btn btn-dim btn-dark" id="'+ ct.arptchart_id+'" onclick="editchart(this.id)"><i class="icon ni ni-edit"></i> Edit</a></li>'+
                            '<li><a class="btn btn-dim btn-light download" id="'+ ct.arptchart_id+'" onclick="showpdf(this.id)"><i class="icon ni ni-file-pdf"></i> Show</a></li>'+
                            '<li><a class="btn btn-dim btn-danger" id="'+ ct.arptchart_id+'" onclick="remove(this.id)"><i class="icon ni ni-delete"></i> Remove</a></li>'+
                        '</ul>'+
                    '</div>'+
                '</div>'+
                '<td>'+no+'</td>'+
...
var ttl=arp.icao + ' AD 2.24 CHARTS RELATED TO AN AERODROME';
$("#arptidname").html(arp.icao + ' ' + arp.city_name +'/'+ arp.arpt_name);
$("#contentitle").html(ttl);
$('#btn_update').click(function() {
   var cc= $("#chart_name").val();
   $("#chart_name").val(cc.toUpperCase());
    $("#chartform").submit();
})
...
function listchartid(id,datachart=null){
    ...
    $.ajax({
        url: '../api/proc/chart',
        type: 'get',
        data:{chart_arpt_ident:arp.arpt_ident,chart_type:charttype},
        success: response => {
            response.data.forEach(c => {
                ...
                if (idx==-1 || datachart == c.chart_id){
                    ...
                    switch (charttype) {
                        case '46':
                        case '47':
                            hsl= '<option value="'+c.chart_id+'">'+id + ' ' + c.cat + ' ('+ proc  +')</option>';
                            break;
                        case '45':
                            hsl= '<option value="'+c.chart_id+'">'+id + ' ' + c.nav + ' ' + c.cat + ' ('+ proc  +')</option>';
                            break;
                        default:
                        hsl= '<option value="'+c.chart_id+'">'+id + '</option>';
                            break;
                    }
...
function getsourcenr(){
    ...
    $.ajax({
        ...
        success: response => {
            response.data.forEach(c => {
                console.log(c)
                $("#pub_date").val(c.pub_date);
                $("#eff_date").val(c.eff_date);

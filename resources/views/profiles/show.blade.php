			...
			@endif
                                ...
                                @if ($user->profile->location)
                                    ...
                                    <dd>
                                        ...
                                        @if(config('settings.googleMapsAPIStatus'))
                                            ...
                                            <div id="map-canvas"><span id="loader"></span></div>
                                        @endif
                                    </dd>
                                @endif
                        ...
                        @if ($user->profile)
                            @if ($currentUser->id == $user->id)
                                {!! HTML::icon_link(URL::to('/profile/'.$currentUser->email.'/edit'), 'fa fa-fw fa-cog', trans('titles.editProfile'), array('class' => 'btn btn-small btn-info btn-block')) !!}
                            @endif
                        @else
                            <p>
                                {{ trans('profile.noProfileYet') }}
                            </p>
                            {!! HTML::icon_link(URL::to('/profile/'.$currentUser->email.'/edit'), 'fa fa-fw fa-plus ', trans('titles.createProfile'), array('class' => 'btn btn-small btn-info btn-block')) !!}
                        @endif
                    </div>
                </div>
            </div>
...
@section('footer_scripts')
    ...
    @if(config('settings.googleMapsAPIStatus'))
        ...
        @include('scripts.leaftlet-maps-geocode-and-map')
    @endif

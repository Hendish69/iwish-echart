...
class CecDepFeatController extends Controller
{
    private $page;
    
    public function __construct()
    {
        $this->page = (object)['title'=>'Departure Features'];
    }
    ...
    {
        $data['page'] = $this->page;
        return View::make('pages.inavcec.depfeat',$data);
    } 
    /**
    ...
    public function update(Request $request, $id)
    {
        ...
        return  response()->json ([
            ...
            $upData = CecDepFeature::firstOrNew(['id' =>  $id]); 
    ...
    {
        $delData = CecDepFeature::find($id);
        $delData->delete();
    }
    // AJAX request
    
    public function getDepFeat(Request $request){ 
     ## Read value
     ...
     // Total records
     $totalRecords = CecDepFeature::select('count(*) as allcount')->count();

     $totalRecordswithFilter = CecDepFeature::select('count(*) as allcount')->where('name', 'like', '%' .$searchValue . '%')->count();

     // Fetch records
     $records = CecDepFeature::orderBy($columnName,$columnSortOrder)
       ->where('cec_depfeature.name', 'like', '%' .$searchValue . '%')
       ->select('cec_depfeature.*')
       ->skip($start)
       ->take($rowperpage)
       ->get();   
     $data_arr = array(); 
